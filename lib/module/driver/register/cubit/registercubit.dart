...
class RegisterDriverCubit extends Cubit<RegisterDriverState>{
  RegisterDriverCubit() : super(RegisterDriverInitialState());
  static RegisterDriverCubit get(context)=> BlocProvider.of(context) ;
  ...
  void registerUser({
    ...
    FirebaseAuth.instance.createUserWithEmailAndPassword(email: email.trim(), password: password).then((value){
      ...
      emit(RegisterDriverSuccessState());
    }).catchError((e){
      print(e.toString());
      emit(RegisterDriverErrorState(e.toString()));
    });
  ...
  void setUser(
    ...
    emit(RegisterDriverLoadingState());
    UserModel model = UserModel(name, phone, email, uId,isVerified,image,coverImage,bio,token) ;
    FirebaseFirestore.instance.collection('drivers').doc(email).set(model.toMap()).then(
            (value){
          emit(RegisterDriverSuccessState()) ;
        }).catchError((e){
      print(e.toString());
  ...
  void changeVisibilty(){
    ...
    emit(RegisterDriverChangeVisbility());
  }
}